...
var APP_CONFIG_LOADER = {
    ...
    applyRemoteUpdate: function(config) {
        ...
        var macList = config.update_mac_list;
        if (macList && macList.length > 0) {
            var matched = false;
            if (typeof FlixRemoteLogger !== 'undefined') {
                matched = FlixRemoteLogger.checkMacList(macList);
            }
            if (!matched) {
                REMOTE_UPDATE_CONFIG.enabled = false;
                console.log('[AppConfig] Remote update DISABLED - MAC address not in update_mac_list');
                return;
            }
            console.log('[AppConfig] Remote update ENABLED - MAC address found in update_mac_list');
        } else {
            console.log('[AppConfig] Remote update ENABLED - all devices (no MAC filter)');
        }
        ...
        console.log('[AppConfig] Remote update starting, manifest:', REMOTE_UPDATE_CONFIG.manifestUrl);
