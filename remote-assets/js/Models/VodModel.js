...
var VodModel ={
    ...
    getFavouriteMovies: function() {
        ...
        return favourites.filter(function(movie) {
            // Only return movies that exist in current catalog
            return that.movies.some(function(catalogMovie) { return catalogMovie.stream_id === movie.stream_id; });
        });
    },
    ...
    pruneInvalidFavorites: function() {
        var that = this;
        var favourites = this.getRecentOrFavouriteMovies('favourite');
        var validFavourites = favourites.filter(function(movie) {
            return that.movies.some(function(catalogMovie) { return catalogMovie.stream_id === movie.stream_id; });
        });
        
        if (validFavourites.length !== favourites.length) {
            this.setRecentOrFavouriteMovies(validFavourites, 'favourite');
            var movie_ids = validFavourites.map(function(movie) { return movie.stream_id; });
